...
Voit luoda oman väripaletin kohdasta **Asetukset /-> Valinnat /-> Värit**. Voit luoda ja nimetä uusia väripaletteja painamalla  ja lisätä olemassaoleviin uusia värejä. Jos sinulla on valmiiksi määritettyjä värejä (RGB- tai HTLM-koodeilla), voit käyttää niitä värien poimimiseen tai valita ne manuaalisesti värikartalta hiirellä.

Painamalla ![](img/harjoitus6/img1.png) voit luoda ja nimetä uuden väripaletin. Värien lisääminen tapahtuu -symbolista.
...
Nyt voit määrittää fontit, koot, tietyt muotoilut, maskit ja muut. Voit tallentaa thext-muodon oletustyyleihin ponnahdusikkunassa, kun tallennat tyylin (Valitse kohtaan Destination: Oletus). Tallennuksen jälkeen voit viedä (export) tyylin lisäyskuvakkeen vierestä ja jakaa sen muille. Hae vain luomasi tyyli luettelosta ja vie se tavalliseen tapaan .xml-tiedostona. Tuonti voidaan tehdä samasta paikasta.

![](img/harjoitus6/tekstinmuotoilu.png)
 
Sitten kun määrität nimiöitä vektoritasolle, luomasi uuden tyylin pitäisi ponnahtaa tyyleihin normaalien tekstimuotoasetusten alapuolella. Esimerkkikuvassa näkyy luotu "test" -tyyli.
