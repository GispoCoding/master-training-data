...
Check again that the **DBF charset** value is **ISO-8859-1**.
...
If necessary, add the *EPSG:4326* code to the *Declared SRS* field. You can also search EPSG codes using the *Find* function.
...
Now add the OpenStreetMap Helsinki road network to GeoServer in the same way as in the previous exercise. The data can be found in the same folder as the **buildings** data, under the name **gis.osm_roads_free_1**.

Remember that before you can add a layer to GeoServer, you need to create a store for it. Name the new store **hki_roads** and the layer created from it **roads** so the layer names remain similar. Remember to also check the coordinate system (**EPSG:4326**) and **DBF charset** (**ISO-8859-1**).
