...
Save the changes by pressing **Apply** first.
...
Now return to the GeoServer interface and click **Browse** above the code box. Find the newly uploaded SLD style on the disk and press **Upload**
...
Then press **Apply** first to save the changes.
...
If you get error messages or the code does not work, ask the trainer for help. Otherwise, press **Apply** to save the style.
...
Press **Save** and preview the layer. Note that when you zoom the map to a scale bigger than 1:32 000, the roads will no longer be visible.

Let's add roads also at a scale of 1:32 000 and larger. This is done by creating a **\<Rule\>** partition with the minimum scale (**\<MaxScaleDenominator\>**) set to **32000**.
...
Press **Apply** and preview. Note that now when you zoom in on the map window (increase the scale), the colour and thickness of the roads will change.
