...
The QGIS user interface is create using Qt, which is a collection
...
Some widgets work as containers and you can add so called child
widgets to them. These can be set to different layout, for example
organized horizontally, vertically or in a grid. This lets you
design how the GUI is organized.
...
When you interact with widgets, the _emit_ different **signalds** which can be
...
In PyQt a signal is an object of type **Signal** wheras a slot is a function
that can be connected to a signals. You can then **emit** the signal wherever
you want.
