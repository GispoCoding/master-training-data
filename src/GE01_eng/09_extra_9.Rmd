...
If you have to include a condition in the comprehension, they are included at the end.
...
With tuples you first have to create a list and create a tuple from it.
...
When using ellipses a generator is created. It evaluates only one element at a time using
the `next` function.
...
Write a list compregension `chose_number` which gets integers 0-20 changed to string so that numbers 9-18 are
excluded.
...
## Topic 2: Lambda function

In Python you can define one-line lambda functions with the following syntax:
...
Create your own decorator funciont `timeit` which measures how much
...
decorated functino such as the docstring. Use the
