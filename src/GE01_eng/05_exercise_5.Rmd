...
* You can use the default settings **expect for**:
	* `Select a name for your plugin`: Test Plugin
	* `Include processing algorithm in your plugin?`: y

5. Deactivate the cookiecutter virtual environment and navigate the the plugin folder:
...
6. Run the Python script which is already inside the folder which creates a new
virtual environment suitable for plugin development.
...
::: hint-box
If your computer has many QGIS installation the script might ask which one of
...
of neede Python packages to a text file:
...
successfull. You can use the `pip freeze` command and you should see
for example `pytest` and `pytest-qgis` in the listed packages.

13. After this we can open Visual Studio Code. Open the `test-plugin.code-workspace`
...
fully set up. You can confirm this by opening the `plygin.py` file inside
...
16. The project includes on test by default which you can check by opening
...
2. Edit the `testplugin/build.py` file and change `profile = "tranining"`
...
2. Open the Plugin menu and from the left click _Installed_ **Test Plugin**
    * Also from the _All_ tab search for **Plugin Reloader** and install it. You need it later on to reload the plugin as you're developing it.
