# Harjoitus 1: Georeferointi

**Harjoituksen sisältö**

Harjoituksessa harjoitellaan georeferointia vastinpisteiden avulla.
...
Lisää kenttään ensin merkki **\>** ja kirjoita tämän jälkeen osoite **Pyynikki** ja tuplaklikkaa esiintulevasta listasta viimeistä osoitetta. Kartan pitäisi zoomata suoraan valittuun paikkaan. Georeferoidaan kartta Tampereen Pyynikistä, voit zoomata karttaa ulospäin, jotta näet Tampereen keskustan ja Pyynikin alueen.


## Georeferoijan toiminta

Avaa georeferoija **Tasot -> Georeferoija**.

![Kuva 1. Georeferoija löytyy- tasot valikosta](img/harjoitus_3/image26_uusi_versio.png)


Kun avaat Georeferoijan, aukeaa uusi ikkuna, jossa itse georeferointi toteutetaan. Georeferoijan ikkunaan voidaan avata rasterikuva **Avaa rasteri**... -painikkeesta ![](img/harjoitus_3/image27.png). Painikkeesta avautuu ikkuna, jonka avulla georeferoitavan kuvan voi valita koneen tiedostojärjestelmästä. Tuettuina on useita kymmeniä eri rasteriformaatteja aina perinteisistä JPEG- ja PNG-formaateista tehokkaisiin JPEG2000- ja ECW-formaatteihin.Voit georeferoida myös PDF:ää, mutta niiden laatu saattaa kärsiä georeferoinnissa.

Kurssihakemistossa on kaksi eri kuvaa, ilmakuva Tampereelta vuodelta 1946 sekä peruskarttarasteri vuodelta 1953. Voit valita kumman vaan georeferoitavaksi. Kun haluttu kuva on valittu tiedostojärjestelmästä ja valinta vahvistettu Open-painikkeesta, avautuu käyttäjälle valittu rasteri näkyviin pienen latauksen jälkeen.
...
Kun tarvittava määrä vastinpisteitä on syötetty, voidaan niitä tarkastella vastinpistetaulun (GCP-taulu) avulla. Mikäli vastinpistetaulu ei näy automaattisesti alareunassa paneelina, sen voi avata Näytä-valikosta löytyvän Paneelit-valikon alta.

Vastinpistetaulussa on nähtävillä kaikki osoitetut vastinpisteet (kuva 10). Tässä esimerkissä on määritelty kymmenen vastinpistettä. Jokaisella vastinpisteelle esitetään taulussa sijainti georeferoitavalla kuvalla (Lähtö X ja Lähtö Y) ja sijainti tukiaineistossa (Kohde. X ja Kohde. Y). Muista valita vastinpisteitä ympäri georeferoitavaa kuvaa georeferoinnin laadun varmistamiseksi. 

![Kuva 11: Esimerkki vastinpisteiden sijainnista ilmakuvassa. Vastinpisteet näkyy punaisella](img/harjoitus_3/image38.png)

Koordinaattien lisäksi okaiselle pisteelle lasketaan virhe X- ja Y-suunnassa. X- ja Y-suuntaisten virhelukujen lisäksi jokaiselle pisteelle on laskettu residuaali, joka kuvaa ko. pisteen kokonaisvirhettä. Ideaalitilanteessa residuaaliarvo olisi mahdollisimman lähellä nollaa.

Huomaa, että virhearvot esitetään pikseleinä. Mikäli jokin vastinpiste eroaa virhearvojen (Residuals/Residuaalit) osalta muista, kannattaa se poistaa ja määritellä uudelleen. Mitä suurempi residuaaliarvo pikselillä on, sitä suurempia virheitä se voi aiheuttaa georeferoinnissa. Tässä esimerkissä voisi siirtää tai määritellä uudelleen pisteet, joiden residuaali on esimerkiksi yli 3. Residuaalit lasketaan uudelleen aina kun teet uusia vastinpisteitä, joten panosta pisteiden sijoittamisen laatuun.
...
Miten alue on muuttunut? Voit verrata taustakarttaa tai uudempaa ilmakuvaa tai satelliittikuvaa vuoden 1946 ilmakuvaan esimerkiksi säätämällä tason läpinäkyvyyttä. Voit myös hyödyntää lisäosaa **MapSwipe**, jonka avulla voit tarkastella karttaa vierekkäin. MapSwipen voi ladata lisäosavalikon kautta.

Jos on aikaa ja haluat harjoitella, voit georeferoida myös toisen kartan alueelta ja verrata niitä keskenään.
